...
{
    float4 diffuse_map_color = diffuse_map.Sample(diffuse_map_sampler_state, pin.texcoord);
    float4 specular_map_color = specular_map.Sample(diffuse_map_sampler_state, pin.texcoord);
    ...
    float3 Kd = diffuse_map_color.rgb;
    float3 D = Diffuse(N, L, C, Kd);
    float3 Ks = specular_map_color.xyz;
