...
void EnemyFarAttack2Task::Run(Enemy* enemy)
{
	...
	case Action::RPUNCH_UPPER:
		...
		}
		{
			...
			{
				StoneParameter& stoneParm = enemy->GetStoneParameter();
				if (!stoneParm.m_stoneAdom->GetStone()->GetExit())
				{
					...
					pos += zAxis * stoneParm.offsetZ;
					pos += xAxis * stoneParm.offsetX;
					...
					stoneParm.m_stoneAdom->SetStone(pos,VECTOR3F(0.0f, 0.0f, 0.0f), normalizeDist);
				}
			}
		}
		break;
