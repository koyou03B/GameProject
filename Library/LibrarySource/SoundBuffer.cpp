...
namespace Source
{
	namespace SoundBuffer
	{
		SoundBuffer::SoundBuffer(LPCWSTR szFilename)
		{
			...
			if (FAILED(hr))
				assert("Could not Initialize Sound Buffer");
		...
		HRESULT SoundBuffer::Initialize(LPCWSTR szFilename)
		{
			...
			if (FAILED(hr))
				assert("Could not find  media file");
#endif

			// Read in the wave file
			hr = LoadWAVAudioFromFile(szFilename, m_waveFiles, m_waveData);
			if (FAILED(hr))
				assert("Could not reading WAV file");

			return S_OK;
		}

		HRESULT SoundBuffer::CreateSourceVoice(IXAudio2* pXaudio2, bool loop)
		{
			HRESULT hr = S_OK;
			m_isLoop = loop;
			...
			hr = pXaudio2->CreateSourceVoice(&src_voice, m_waveData.waveFormaTex);
			if (FAILED(hr))
			{
			...
			buffer.pAudioData = m_waveData.startAudio;
			buffer.Flags = XAUDIO2_END_OF_STREAM;  // tell the source voice not to expect any data after this buffer
			buffer.AudioBytes = m_waveData.audioBytes;

			if (loop)
			{
				buffer.LoopBegin = m_waveData.loopStart;
				buffer.LoopLength = m_waveData.loopLength;
			...
			if (m_waveData.seek)
			{
				...
				xwmaBuffer.pDecodedPacketCumulativeBytes = m_waveData.seek;
				xwmaBuffer.PacketCount = m_waveData.seekCount;
				if (FAILED(hr = m_pSourceVoice.front()->SubmitSourceBuffer(&buffer, &xwmaBuffer)))
				{
					m_pSourceVoice.front()->DestroyVoice();
					assert("Error SubmitSourceBuffer);
						return hr;
				}
			}
#else
			...
			{
				src_voice->DestroyVoice();
				assert("Error submitting source buffer");
				return hr;
			}

			m_pSourceVoice.push(src_voice);
		...
		void SoundBuffer::Update()
		{
			...
			{
				m_pSourceVoice.front()->GetState(&state);
				if (state.BuffersQueued == 0)
				{
					m_pSourceVoice.front()->DestroyVoice();
					m_pSourceVoice.front() = nullptr;
					m_pSourceVoice.pop();
				}
			}

			for (size_t i = 1; i < m_pSourceVoice.size(); ++i)
			{
				// Let the sound play
				m_pSourceVoice.front()->GetState(&state);
			}
		...
		HRESULT SoundBuffer::LoadWAVAudioFromFile(const wchar_t* szFileName, std::unique_ptr<uint8_t[]>& wavData, WAVData& result)
		{
			if (!szFileName)
				return E_INVALIDARG;
			...
			bool dpds, seek;
			hr = WaveFindFormatAndData(wavData.get(), bytesRead, &result.waveFormaTex, &result.startAudio, &result.audioBytes, dpds, seek);
			if (FAILED(hr))
				return hr;
